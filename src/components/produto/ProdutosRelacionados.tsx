...
export function ProdutosRelacionados({ categoriaAtual, idAtual }: Props) {
  ...
  function handleAdicionar(e: React.MouseEvent, p: Product) {
    ...
    // Para relacionados, adicionamos a variação padrão se houver
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {produtos.map(p => (
            <Link key={p.id} href={`/produto/${p.id}`} className="group flex flex-col h-full">
              <div className="relative aspect-square rounded-[1.5rem] overflow-hidden bg-white shadow-sm border border-primary/5 mb-4">
                <Image
                  src={p.imageUrl}
                  alt={p.name}
                  fill
                ...
                <div className="absolute inset-x-0 bottom-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300 hidden md:block">
                  <button
                    ...
                    className={`w-full py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-xl transition-all ${
                      adicionados[p.id] 
                ...
                {/* Mobile: Botão visível */}
                <button
                  onClick={(e) => handleAdicionar(e, p)}
                  className="absolute bottom-2 right-2 md:hidden h-8 w-8 rounded-full bg-primary text-white flex items-center justify-center shadow-lg active:scale-90 transition-transform"
                >
              ...
              <div className="flex-1 space-y-1">
                <p className="text-[8px] font-bold text-primary/40 uppercase tracking-widest">{p.category}</p>
                <h3 className="font-poppins font-normal text-xs text-primary leading-tight line-clamp-2 min-h-[2.5em] group-hover:text-primary/70 transition-colors">
                  {p.name}
                </h3>
